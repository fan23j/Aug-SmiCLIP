...
class SimpleTokenizer(object):
    ...
    def eval_tokenize(
        self, texts: Union[str, List[str]], context_length=77
    ) -> torch.LongTensor:
        if isinstance(texts, str):
            texts = [texts]
    ...
    def __call__(self, texts, context_length=77):
        if isinstance(texts, str):  # training time
            texts = [texts]
            ...
            texts = [
                ...
                self.strong_augment(random.choice(texts)),
            ]
...
def eval_tokenize(texts: Union[str, List[str]], context_length: int = 77) -> torch.LongTensor:
    return _tokenizer.eval_tokenize(texts)
